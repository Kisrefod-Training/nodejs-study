import {Collaborator, ParsedData} from "../src/gitParserTypes";
...
let branchNames:{ status: number, data: Array<{name: string}> } = {
    status: 200,
    data: []
};
...
function getExpectedPullReqs() {
    ...
    collaborators.forEach(collaborator => {
        const items = pullReqs.data.items
...
function getExpectedCommits() {
    ...
    Object.values(branchCommits).forEach(branch => {
        branch.data.forEach(data => {
            ...
            if(!result.find(elem => elem.ID === commitData.sha)) result.push({
